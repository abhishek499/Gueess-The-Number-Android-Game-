...
const gnerateRandomBetween = (min, max, exclude) => {
  min = Math.ceil(min);
  max = Math.floor(max);
...
const GameScreen = (props) => {
  const initialGuess = gnerateRandomBetween(1, 100, props.userChoice);
  ...
  const nextGameHandler = (direction) => {
    ...
    const nextNumber = gnerateRandomBetween(
      currentLow.current,
      currentHigh.current,
      currentGuess
    );
